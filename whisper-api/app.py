...
from chat_shell import process_message
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        task_dict[item_id] = {"filename": f"{item_id}.m4a", "response": None, "error": False}

        # Start transcription in background
        thread = threading.Thread(target=process_audio, args=(item_id, file_path))
        thread.start()
...
def poll(task_id):
    if task_id not in task_dict:
        return {"ready": False, "error": "Task not found"}, 404

    task = task_dict[task_id]
    if task["response"] is None:
        return {"ready": False}, 200

    return {"ready": True, "message": task["response"], "error": task.get("error", False)}, 200
